...
const DisplayResult = ({ data }) => {
    ...
    // Original data processing remains unchanged
    ...
    for (const key in frequent_patterns) {
        ...
        if (items.length > 1) {
            ...
            singleItemsets[items.join(", ")] = count;
        }
    }
    ...
    const sortedSingleItemsets = Object.entries(singleItemsets)
        .sort(([, a], [, b]) => b - a)
        .reduce((r, [k, v]) => ({...r, [k]: v}), {});
    ...
    const splitIndex = Math.ceil(sortedData.length * 0.40);
    ...
    return (
        <><h1>FrequentItemsetsCountChart</h1>
            ...
            <div>
                <SupportCountForItemsets itemsAndCounts={sortedSingleItemsets}/>
            </div>
                ...
                >
                    <div>
                        {Object.keys(sortedItemsAndCounts).map((key) => (
                            <div
                                key={key}
                                style={{backgroundColor: "teal", padding: "20px"}}
                            >
